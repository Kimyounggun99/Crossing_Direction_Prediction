...
class EmbedPosEnc(nn.Module):
    def __init__(self, input_size, d_model,deivce):
        ...
        self.pos_enc = PositionalEncoding(d_model).to(device) # 位置编码
    ...
    def forward(self, x, token):
        ...
        y = self.embedding(x) 
        token = repeat(token, '() s e -> b s e', b=b)
...
class AttentionBlocks(nn.Module):
    def __init__(self, d_model, num_heads, rate=0.3, layer_norm_eps=1e-5,device=None):
        super(AttentionBlocks, self).__init__()

        self.att = nn.MultiheadAttention(d_model, num_heads=num_heads, batch_first=True).to(device) # 多头注意力
        self.drop = nn.Dropout(rate) 
        self.norm = nn.LayerNorm(d_model, eps=layer_norm_eps).to(device) # 归一化

    def forward(self, x, y=None):
        y = x if y is None else y
      
        att_out, att_w = self.att(x, y, y) 
